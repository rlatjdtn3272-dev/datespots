...
export default function App() {
  ...
  // ── URL import (여자친구 링크로 데이터 가져오기)
  useEffect(() => {
    const hash = window.location.hash;
    if (hash.startsWith("#import=")) {
      try {
        const imported = JSON.parse(decodeURIComponent(hash.slice(8)));
  ...
  // ── 내보내기 (링크 생성)
  const exportLink = () => {
    const encoded = encodeURIComponent(JSON.stringify(places));
    const url = `${window.location.origin}${window.location.pathname}#import=${encoded}`;
    navigator.clipboard.writeText(url).then(() => showToast("📋 링크가 복사됐어요! 여자친구에게 보내세요 💌")).catch(() => showToast("링크 복사 실패 - 직접 복사해주세요"));
  };

  // ── AI 분석
  const analyzeWithAI = async () => {
    if (!addName.trim()) { setAiError("가게 이름을 입력해주세요."); return; }
...
{
  "region": "지역명 (예: 홍대·연남, 강남·서초 등)",
  "search": "네이버 검색어 (가게명+지역 3-4단어)",
  "tags": ["food/bar/cafe/activity/market 중 해당", "한식/일식/양식/분식 중 하나(식당이면)"],
  "concepts": ["분위기/특징 키워드 3-5개"],
  "isNaeng": false
}
isNaeng은 평양냉면 전문점이면 true.` }]
      ...
      setAiPreview(parsed);
    } catch { setAiError("분석 오류가 발생했어요. 다시 시도해주세요."); }
    setAiLoading(false);
  };

  const confirmAdd = () => {
    if (!aiPreview) return;
    const newPlace = { id:"u"+Date.now(), name:addName.trim(), addr:addAddr.trim()||aiPreview.region+" 일대", ...(addNote.trim()?{note:addNote.trim()}:{}), ...aiPreview };
    const updated = [...places, newPlace];
    save(updated);
    setAddName(""); setAddAddr(""); setAddNote(""); setAiPreview(null);
    setView("list");
    showToast("✅ 추가됐어요!");
  };
  ...
  const FBtn = ({label,active,onClick})=>(
    <button onClick={onClick} style={{fontSize:12,padding:"6px 14px",borderRadius:999,whiteSpace:"nowrap",border:active?"none":"1px solid #E5E7EB",background:active?"#111":"#fff",color:active?"#fff":"#6B7280",fontWeight:active?600:400,cursor:"pointer",flexShrink:0}}>
      {label}
    </button>
  );
  ...
  if (view==="add") return (
    <div style={s}>
      <div style={{background:"#fff",padding:"52px 20px 12px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #F3F4F6",position:"sticky",top:0,zIndex:10}}>
        <button onClick={()=>{setView("list");setAiPreview(null);setAiError("");}} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#374151",padding:0,lineHeight:1}}>←</button>
        <span style={{fontSize:17,fontWeight:700,color:"#111"}}>새 장소 추가</span>
      </div>
      <div style={{padding:"20px 20px 60px"}}>
        <div style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 1px 6px rgba(0,0,0,0.06)",marginBottom:16}}>
          {[["가게 이름 *",addName,setAddName,"예) 트릴로지 합정",true],["주소",addAddr,setAddAddr,"예) 서울 마포구 희우정로 19",false],["메모",addNote,setAddNote,"예) 월 휴무, 웨이팅 있음",false]].map(([label,val,setter,ph,req])=>(
            <div key={label} style={{marginBottom:16}}>
              <label style={{fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:6}}>{label}</label>
              <input value={val} onChange={e=>{setter(e.target.value);if(req)setAiPreview(null);}} placeholder={ph}
                style={{width:"100%",padding:"12px 14px",borderRadius:10,border:"1.5px solid #E5E7EB",fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"inherit"}}/>
            </div>
          ))}
          <button onClick={analyzeWithAI} disabled={aiLoading||!addName.trim()}
            style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:addName.trim()?"linear-gradient(135deg,#667EEA,#764BA2)":"#E5E7EB",color:addName.trim()?"#fff":"#9CA3AF",fontSize:15,fontWeight:700,cursor:addName.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
        ...
        {aiPreview&&(
          <div style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 1px 6px rgba(0,0,0,0.06)",border:"2px solid #667EEA"}}>
            ...
            <div style={{marginBottom:20}}><div style={{fontSize:12,color:"#9CA3AF",marginBottom:6}}>컨셉 키워드</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{aiPreview.concepts?.map(c=><Chip key={c} label={c}/>)}</div></div>
            <button onClick={confirmAdd} style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:"#111",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>목록에 추가하기 ✓</button>
          </div>
        )}
      </div>
  ...
  if (view==="detail"&&selectedPlace) {
    ...
    return (
      <div style={s}>
        <div style={{background:"#fff",padding:"52px 20px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #F3F4F6",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{padding:"20px"}}>
          <div style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 1px 6px rgba(0,0,0,0.06)",marginBottom:14}}>
            ...
            <div style={{fontSize:14,color:"#374151",marginBottom:p.note?16:0,lineHeight:1.6}}>{p.addr}</div>
            {p.note&&<><div style={{fontSize:12,color:"#9CA3AF",marginBottom:4,marginTop:12}}>메모</div><div style={{fontSize:14,color:"#374151",marginBottom:0,lineHeight:1.6}}>{p.note}</div></>}
    ...
    <div style={s}>
      ...
      <div style={{background:"#fff",padding:"52px 20px 10px",borderBottom:"1px solid #F3F4F6",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div>
            <div style={{fontSize:21,fontWeight:800,color:"#111",letterSpacing:"-0.5px"}}>📍 우리 가볼 곳</div>
