...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (hash.startsWith("#share=")) {
      ...
      fetch(`${url}/get/share_${code}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then(r => r.json())
      .then(data => {
        if (data.result) {
          const imported = JSON.parse(data.result);
  ...
  const exportLink = async () => {
    ...
    try {
      ...
      const url = import.meta.env.VITE_KV_REST_API_URL;
      const token = import.meta.env.VITE_KV_REST_API_TOKEN;
      await fetch(`${url}/set/share_${code}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ value: JSON.stringify(places), ex: 604800 })
      });
